...
import com.example.TripPick_backend_PubDataBridge.domain.event.TourCourseInfoEvent;
import com.example.TripPick_backend_PubDataBridge.dto.response.TourCourseInfoResponse;
import com.example.TripPick_backend_PubDataBridge.event.producer.TourCourseInfoProducer;
...
public class TourCourseInfoService {
    ...
    private final TourCourseInfoProducer tourCourseInfoProducer;
    ...
    public void fetchAndSendInfo(String serviceKey, String contentTypeId, String contentId) {
        ...
        TourCourseInfoResponse response = null;

        try {
            response = restTemplate.getForObject(url, TourCourseInfoResponse.class);
        ...
        for (TourCourseInfoResponse.Item dto : response.getResponse().getBody().getItems().getItem()) {
            TourCourseInfo entity = TourCourseInfo.builder()
                    .contentid(dto.getContentid())
                    .distance(dto.getDistance())
                    .infocentertourcourse(dto.getInfocentertourcourse())
                    .schedule(dto.getSchedule())
                    .taketime(dto.getTaketime())
                    .theme(dto.getTheme())
                    .build();

            TourCourseInfoEvent event = TourCourseInfoEvent.fromEntity("tourcourseinfo", entity);
            tourCourseInfoProducer.send(KafkaConfig.topics.TOURCOURSEINFO, event);
        }
    }
